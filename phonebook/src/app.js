  ...
  const handleSave = (event) => {
    ...
    if (exists && exists.number !== newNumber) {
      if (
        ...
        personService
          .update(exists.id, exists)
          .then((updated) => {
            setNotification(`Person '${updated.name}' has a new number`);
            setTimeout(() => {
    ...
    personService
      .create(person)
      .then((returnedPerson) => {
        setNotification(`Added '${returnedPerson.name}' to phone book`);
        setTimeout(() => {
          setNotification(null);
        }, 5000);
      ...
      .catch((error) => {
        setError(`Person validation failed: ${error}`);
      });
  };
